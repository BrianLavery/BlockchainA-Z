...
contract briancoin_ico {
    ...
    // External constant is because address is outside contract and never changes - applies to parameters
    function equity_in_briancoins(address investor) external constant returns (uint) {
        return equity_briancoins(investor); // Mapping above takes address of investor as input
    }
